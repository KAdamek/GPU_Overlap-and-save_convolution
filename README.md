...
Parameters if input type is 'f' - file input provided by user
 ...
 6) number of filters
 Example: CONV.exe f signal.dat filter.dat output.dat 2048 32

Parameters if input type is 'r' - random input generated by the code
 2) Signal length in number of time samples
 3) Filter length in samples
 4) Convolution length in samples
 5) Number of filters
 6) number of GPU kernel runs
 Example: CONV.exe r 2097152 193 2048 32 10
		
In case of AAFFT implementation there is no universal segment size. The maximum segment size is 4096. 
