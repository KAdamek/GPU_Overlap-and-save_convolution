...
void Generate_signal(float2 *h_input, int signal_length){
...
int main(int argc, char* argv[]) {
	...
	if (input_type == 'f' && argc>=6 && argc<9) {
		...
		if(argc==8) nRuns = strtol(argv[7],&pEnd,10);
		else nRuns=1;
		reglim = 0;
	}
	else if (input_type == 'r' && argc>=6 && argc<8) {
		...
		if(argc>=6) nRuns = strtol(argv[6],&pEnd,10);
		else nRuns=1;
	}
	else {
		...
		printf(" 7) number of GPU kernel runs (optional)\n");
		printf(" Example: CONV.exe f signal.dat filter.dat output.dat 2048 32 10\n");
		printf("----------------------------------\n");
		...
		printf(" Example: CONV.exe r 2097152 192 2048 32 10\n");
        return 1;
	}
	...
	CONV_cuFFT.Assign(signal_length, filter_length, nFilters, nRuns, reglim, convolution_length, nFilters, "CONV_kFFT.dat", "two");
	...
	if( useful_part_size<=1) {
		printf("Filter length is too long. Increase FFT length.\n");
		exit(1);
	}
	if( convolution_length==4096 ) {
		printf("Convolution length is too long for this GPU kernel.\n");
		exit(1);
	}
	
	if (input_type == 'r') {
		if (nFilters%2!=0) nFilters++; //number of filters must be divisible by 2
	...
	//----------------> GPU kernel
	int kernel_type=2; //two filters per iteration
