...
int GPU_CONV(float2 *h_input, float2 *h_output, float2 *h_filters, int useful_part_size, int offset, int nConvolutions, int nFilters, int nRuns, int kernel_type, double *execution_time);
...
int main(int argc, char* argv[]) {
	...
	if (input_type == 'f' && argc>=6 && argc<8) {
		...
		printf(" 6) number of registers (--maxrregcount flag for nvcc) (optional)\n");
	...
	CONV_cuFFT.Assign(nTimesamples, filter_length, nFilters, nRuns, reglim, CONV_SIZE, nFilters, "CONV_kFFT.dat", "two");
	...
	GPU_CONV(h_input, h_output, h_filters_padded, useful_part_size, offset, nConvolutions, nFilters, nRuns, kernel_type, &execution_time);
