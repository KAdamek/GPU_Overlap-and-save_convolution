...
public:
	...
	static const int conv_length = -1;
...
class FFT_256 : public FFT_Params {
	...
	static const int conv_length = 256;
	static const int conv_length_quarter = 64;
	static const int conv_length_half = 128;
	static const int conv_length_three_quarters = 192;
};

class FFT_512 : public FFT_Params {
	public:
	static const int fft_exp = 9;
	static const int conv_length = 512;
	static const int conv_length_quarter = 128;
	static const int conv_length_half = 256;
	static const int conv_length_three_quarters = 384;
};

class FFT_1024 : public FFT_Params {
	public:
	static const int fft_exp = 10;
	static const int conv_length = 1024;
	static const int conv_length_quarter = 256;
	static const int conv_length_half = 512;
	static const int conv_length_three_quarters = 768;
};

class FFT_2048 : public FFT_Params {
	public:
	static const int fft_exp = 11;
	static const int conv_length = 2048;
	static const int conv_length_quarter = 512;
	static const int conv_length_half = 1024;
	static const int conv_length_three_quarters = 1536;
};

class FFT_4096 : public FFT_Params {
	public:
	static const int fft_exp = 12;
	static const int conv_length = 4096;
	static const int conv_length_quarter = 1024;
	static const int conv_length_half = 2048;
	static const int conv_length_three_quarters = 3072;
};
...
__inline__ __device__ void FFT_CT_DIT_4elem_no_reorder(float2 *s_input){
	...
	B_load_id = threadIdx.x + const_params::conv_length/4;
	A_n=threadIdx.x;
	B_n=threadIdx.x + const_params::conv_length/4;
    ...
    A_load_id &= const_params::conv_length-1;
	B_load_id &= const_params::conv_length-1;
	...
	s_input[threadIdx.x + const_params::conv_length/2]   = B_DFT_value;
	s_input[threadIdx.x + const_params::conv_length/4]   = C_DFT_value;
	s_input[threadIdx.x + 3*const_params::conv_length/4] = D_DFT_value;
	__syncthreads();
	#endif
...
template<class const_params>
__inline__ __device__ void FFT_CT_DIF_4elem_no_reorder(float2 *s_input){
	float2 A_DFT_value, B_DFT_value, C_DFT_value, D_DFT_value;
	float2 W;
	float2 Aftemp, Bftemp, Cftemp, Dftemp;
	...
	//-----> FFT
	//-->
	PoTm1 = (const_params::conv_length>>1);
	PoT   = const_params::conv_length;
...
__global__ void k_customFFT_GPU_forward(float2 *d_input, float2* d_output) {
	...
	s_input[threadIdx.x]                                            = d_input[threadIdx.x + blockIdx.x*const_params::conv_length];
	s_input[threadIdx.x + const_params::conv_length_quarter]        = d_input[threadIdx.x + blockIdx.x*const_params::conv_length + const_params::conv_length_quarter];
	s_input[threadIdx.x + const_params::conv_length_half]           = d_input[threadIdx.x + blockIdx.x*const_params::conv_length + const_params::conv_length_half];
	s_input[threadIdx.x + const_params::conv_length_three_quarters] = d_input[threadIdx.x + blockIdx.x*const_params::conv_length + const_params::conv_length_three_quarters];
	
	__syncthreads();
	FFT_CT_DIF_4elem_no_reorder<const_params>(s_input);
	
	__syncthreads();
	d_output[threadIdx.x + blockIdx.x*const_params::conv_length]                                            = s_input[threadIdx.x];
	d_output[threadIdx.x + blockIdx.x*const_params::conv_length + const_params::conv_length_quarter]        = s_input[threadIdx.x + const_params::conv_length_quarter];
	d_output[threadIdx.x + blockIdx.x*const_params::conv_length + const_params::conv_length_half]           = s_input[threadIdx.x + const_params::conv_length_half];
	d_output[threadIdx.x + blockIdx.x*const_params::conv_length + const_params::conv_length_three_quarters] = s_input[threadIdx.x + const_params::conv_length_three_quarters];
}
...
__device__ __inline__ void prepare_signal_4elem(float2* s_signal, float2 const* __restrict__ d_input_signal, int signal_length, int useful_part_size, int offset) {
	...
	s_signal[threadIdx.x + const_params::conv_length_quarter].x        = 0;
	s_signal[threadIdx.x + const_params::conv_length_quarter].y        = 0;
	s_signal[threadIdx.x + const_params::conv_length_half].x           = 0;
	s_signal[threadIdx.x + const_params::conv_length_half].y           = 0;
	s_signal[threadIdx.x + const_params::conv_length_three_quarters].x = 0;
	s_signal[threadIdx.x + const_params::conv_length_three_quarters].y = 0;
	
	if( pos>0 && pos<signal_length ) 
		s_signal[threadIdx.x] = __ldg(&d_input_signal[pos]);
	
	if( (pos + const_params::conv_length_quarter)>0 && (pos + const_params::conv_length_quarter)<signal_length ) 
		s_signal[threadIdx.x + const_params::conv_length_quarter] = __ldg(&d_input_signal[pos + const_params::conv_length_quarter]);
		
	if( (pos + const_params::conv_length_half)>0 && (pos + const_params::conv_length_half)<signal_length ) 	
		s_signal[threadIdx.x + const_params::conv_length_half] = __ldg(&d_input_signal[pos + const_params::conv_length_half]);
	
	if( (pos + const_params::conv_length_three_quarters)>0 && (pos + const_params::conv_length_three_quarters)<signal_length ) 
		s_signal[threadIdx.x + const_params::conv_length_three_quarters] = __ldg(&d_input_signal[pos + const_params::conv_length_three_quarters]);
}
...
__global__ void k_GPU_conv_OLS_via_customFFT(
	...
	FFT_CT_DIF_4elem_no_reorder<const_params>(s_input_1);
	
	// Storing FFTed signal for reuse
	signal[0]=s_input_1[threadIdx.x];
	signal[1]=s_input_1[threadIdx.x + const_params::conv_length_quarter];
	signal[2]=s_input_1[threadIdx.x + const_params::conv_length_half];
	signal[3]=s_input_1[threadIdx.x + const_params::conv_length_three_quarters];
	
	for(t=0; t<nFilters; t++){
		// Loading filters
		pos = t*const_params::conv_length + threadIdx.x;
		r_filter_1[0]=__ldg(&d_filters[pos]);
		r_filter_1[1]=__ldg(&d_filters[pos + const_params::conv_length_quarter]);
		r_filter_1[2]=__ldg(&d_filters[pos + const_params::conv_length_half]);
		r_filter_1[3]=__ldg(&d_filters[pos + const_params::conv_length_three_quarters]);
		...
		s_input_1[threadIdx.x + const_params::conv_length_quarter].x        = r_filter_1[1].x*signal[1].x - r_filter_1[1].y*signal[1].y;
		s_input_1[threadIdx.x + const_params::conv_length_quarter].y        = r_filter_1[1].x*signal[1].y + r_filter_1[1].y*signal[1].x;
		s_input_1[threadIdx.x + const_params::conv_length_half].x           = r_filter_1[2].x*signal[2].x - r_filter_1[2].y*signal[2].y;
		s_input_1[threadIdx.x + const_params::conv_length_half].y           = r_filter_1[2].x*signal[2].y + r_filter_1[2].y*signal[2].x;
		s_input_1[threadIdx.x + const_params::conv_length_three_quarters].x = r_filter_1[3].x*signal[3].x - r_filter_1[3].y*signal[3].y;
		s_input_1[threadIdx.x + const_params::conv_length_three_quarters].y = r_filter_1[3].x*signal[3].y + r_filter_1[3].y*signal[3].x;
		
		__syncthreads();
		
		//----------> Inverse FFT
		FFT_CT_DIT_4elem_no_reorder<const_params>(s_input_1);
		//----------<
		...
		if( (threadIdx.x + const_params::conv_length_quarter)>=offset && (threadIdx.x + const_params::conv_length_quarter)<(useful_part_size+offset) ) {
			d_output_plane[pos + const_params::conv_length_quarter - offset] = s_input_1[threadIdx.x + const_params::conv_length_quarter];
		}
		if( (threadIdx.x + const_params::conv_length_half)>=offset && (threadIdx.x + const_params::conv_length_half)<(useful_part_size+offset) ) {
			d_output_plane[pos + const_params::conv_length_half - offset] = s_input_1[threadIdx.x + const_params::conv_length_half];
		}
		if( (threadIdx.x + const_params::conv_length_three_quarters)>=offset && (threadIdx.x + const_params::conv_length_three_quarters)<(useful_part_size+offset) ) {
			d_output_plane[pos + const_params::conv_length_three_quarters - offset] = s_input_1[threadIdx.x + const_params::conv_length_three_quarters];
		}
	}
}
...
__global__ void k_GPU_conv_OLS_via_customFFT_2filters(
			...
			int nConvolutions, 
			int nFilters) {
	__shared__ float2 s_input_1[const_params::conv_length];
	__shared__ float2 s_input_2[const_params::conv_length];
	float2 r_filter_1[4];
	float2 r_filter_2[4];
	float2 signal[4];
	int pos, t;
	...
	for(t=0; t<(nFilters>>1); t++){
		// Loading filters
		pos = 2*t*const_params::conv_length + threadIdx.x;
		...
		r_filter_2[0]=__ldg(&d_filters[pos + const_params::conv_length]);
		r_filter_2[1]=__ldg(&d_filters[pos + const_params::conv_length + const_params::conv_length_quarter]);
		r_filter_2[2]=__ldg(&d_filters[pos + const_params::conv_length + const_params::conv_length_half]);
		r_filter_2[3]=__ldg(&d_filters[pos + const_params::conv_length + const_params::conv_length_three_quarters]);
		...
		s_input_2[threadIdx.x + const_params::conv_length_quarter].x        = r_filter_2[1].x*signal[1].x - r_filter_2[1].y*signal[1].y;
		s_input_2[threadIdx.x + const_params::conv_length_quarter].y        = r_filter_2[1].x*signal[1].y + r_filter_2[1].y*signal[1].x;
		s_input_2[threadIdx.x + const_params::conv_length_half].x           = r_filter_2[2].x*signal[2].x - r_filter_2[2].y*signal[2].y;
		s_input_2[threadIdx.x + const_params::conv_length_half].y           = r_filter_2[2].x*signal[2].y + r_filter_2[2].y*signal[2].x;
		s_input_2[threadIdx.x + const_params::conv_length_three_quarters].x = r_filter_2[3].x*signal[3].x - r_filter_2[3].y*signal[3].y;
		s_input_2[threadIdx.x + const_params::conv_length_three_quarters].y = r_filter_2[3].x*signal[3].y + r_filter_2[3].y*signal[3].x;
		...
		if( (threadIdx.x + const_params::conv_length_quarter)>=offset && (threadIdx.x + const_params::conv_length_quarter)<(useful_part_size+offset) ) {
			d_output_plane[pos + const_params::conv_length_quarter - offset] = s_input_2[threadIdx.x + const_params::conv_length_quarter];
		}
		if( (threadIdx.x + const_params::conv_length_half)>=offset && (threadIdx.x + const_params::conv_length_half)<(useful_part_size+offset) ) {
			d_output_plane[pos + const_params::conv_length_half - offset] = s_input_2[threadIdx.x + const_params::conv_length_half];
		}
		if( (threadIdx.x + const_params::conv_length_three_quarters)>=offset && (threadIdx.x + const_params::conv_length_three_quarters)<(useful_part_size+offset) ) {
			d_output_plane[pos + const_params::conv_length_three_quarters - offset] = s_input_2[threadIdx.x + const_params::conv_length_three_quarters];
		}
	}
}
...
void conv_OLS_customFFT(float2 *d_input_signal, float2 *d_output_plane, float2 *d_filters, int signal_length, int convolution_length, int useful_part_size, int offset, int nConvolutions, int nFilters){
	dim3 gridSize(nConvolutions, 1, 1);
	dim3 blockSize(convolution_length/4, 1, 1);
	
	switch(convolution_length) {
		case 256:
			k_GPU_conv_OLS_via_customFFT<FFT_256><<<gridSize, blockSize, convolution_length*8>>>(d_input_signal, d_output_plane, d_filters, signal_length, useful_part_size, offset, nConvolutions, nFilters);
			break;
			
		case 512:
			k_GPU_conv_OLS_via_customFFT<FFT_512><<<gridSize, blockSize, convolution_length*8>>>(d_input_signal, d_output_plane, d_filters, signal_length, useful_part_size, offset, nConvolutions, nFilters);
			break;
		
		case 1024:
			k_GPU_conv_OLS_via_customFFT<FFT_1024><<<gridSize, blockSize, convolution_length*8>>>(d_input_signal, d_output_plane, d_filters, signal_length, useful_part_size, offset, nConvolutions, nFilters);
			break;

		case 2048:
			k_GPU_conv_OLS_via_customFFT<FFT_2048><<<gridSize, blockSize, convolution_length*8>>>(d_input_signal, d_output_plane, d_filters, signal_length, useful_part_size, offset, nConvolutions, nFilters);
			break;
			
		case 4096:
			k_GPU_conv_OLS_via_customFFT<FFT_4096><<<gridSize, blockSize, convolution_length*8>>>(d_input_signal, d_output_plane, d_filters, signal_length, useful_part_size, offset, nConvolutions, nFilters);
			break;
...
void convolution_via_customFFT_benchmark(float2 *d_input_signal, float2 *d_output_plane, float2 *d_filters, int signal_length, int convolution_length, int useful_part_size, int offset, int nConvolutions, int nFilters, double *CONV_time, int kernel_type){
	GpuTimer timer;
	...
	CONV_init();
	if(kernel_type==1){
		conv_OLS_customFFT(d_input_signal, d_output_plane, d_filters, signal_length, convolution_length, useful_part_size, offset, nConvolutions, nFilters);
	}
...
int GPU_convolution_OLS_customFFT(float2 *h_input_signal, float2 *h_output_plane, float2 *h_filters, int signal_length, int convolution_length, int filter_length, int nFilters, int nRuns, int kernel_type, double *execution_time){
	...
	//---------> CONV calculation
		...
		for(int f=0; f<nRuns; f++){
			checkCudaErrors(cudaMemcpy(d_input_signal, h_input_signal, input_size*sizeof(float2), cudaMemcpyHostToDevice));
			checkCudaErrors(cudaMemcpy(d_filters, h_filters, template_size*sizeof(float2), cudaMemcpyHostToDevice));
			convolution_via_customFFT_benchmark(d_input_signal, d_output_plane, d_filters, signal_length, convolution_length, useful_part_size, offset, nConvolutions, nFilters, &total_CONV_kFFT_time, kernel_type);
