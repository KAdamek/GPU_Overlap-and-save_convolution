...
int GPU_convolution_OLS_customFFT(float *h_input_signal, float *h_output_plane, float *h_filters, int signal_length, int convolution_length, int filter_length, int nFilters, int nRuns, float h, int offset_modifier, int device, double *execution_time);
...
int main(int argc, char* argv[]) {
	...
	//----------------> GPU kernel
	GPU_convolution_OLS_customFFT(h_input, h_output, h_filters_padded, signal_length, convolution_length, filter_length, nFilters, nRuns, h, offset_modifier, DEVICEID, &execution_time);
	...
	if(CHECK){
		...
		Full_CONV_check(h_output, h_input, h_filters, signal_length, filter_length, useful_part_size, (filter_length>>1), convolution_length, nConvolutions, nFilters, h, &total_error, &mean_error);
