...
__global__ void k_GPU_conv_OLS_R2R_via_customFFT(
	...
	for(t=0; t<nFilters; t++){
		...
		if( threadIdx.x>=offset && threadIdx.x<(useful_part_size+offset) ) {
			#ifdef POST_PROCESS
			d_output_plane[pos - offset] = post_process((float *) s_input_1, (threadIdx.x<<1), (offset<<1), h);
			#else
		...
		if( (threadIdx.x + const_params::fft_quarter)>=offset && (threadIdx.x + const_params::fft_quarter)<(useful_part_size+offset) ) {
			#ifdef POST_PROCESS
			d_output_plane[pos + const_params::fft_quarter - offset] = post_process((float *) s_input_1, ((threadIdx.x + const_params::fft_quarter)<<1), (offset<<1), h);
			#else
		...
		if( (threadIdx.x + const_params::fft_half)>=offset && (threadIdx.x + const_params::fft_half)<(useful_part_size+offset) ) {
			#ifdef POST_PROCESS
			d_output_plane[pos + const_params::fft_half - offset] = post_process((float *) s_input_1, ((threadIdx.x + const_params::fft_half)<<1), (offset<<1), h);
			#else
		...
		if( (threadIdx.x + const_params::fft_threequarters)>=offset && (threadIdx.x + const_params::fft_threequarters)<(useful_part_size+offset) ) {
			#ifdef POST_PROCESS
			d_output_plane[pos + const_params::fft_threequarters - offset] = post_process((float *) s_input_1, ((threadIdx.x + const_params::fft_threequarters)<<1), (offset<<1), h);
			#else
...
void conv_OLS_R2R_customFFT(float *d_input_signal, float *d_output_plane, float2 *d_filters, int signal_length, int convolution_length, int useful_part_size, float h, int offset, int nConvolutions, int nFilters){
	...
	switch(convolution_length) {
		case 512:
			k_GPU_conv_OLS_R2R_via_customFFT<FFT_256><<<gridSize, blockSize>>>(d_input_signal, d_output_plane, d_filters, signal_length, useful_part_size, h, offset, nConvolutions, nFilters);
			break;
		
		case 1024:
			k_GPU_conv_OLS_R2R_via_customFFT<FFT_512><<<gridSize, blockSize>>>(d_input_signal, d_output_plane, d_filters, signal_length, useful_part_size, h, offset, nConvolutions, nFilters);
			break;

		case 2048:
			k_GPU_conv_OLS_R2R_via_customFFT<FFT_1024><<<gridSize, blockSize>>>(d_input_signal, d_output_plane, d_filters, signal_length, useful_part_size, h, offset, nConvolutions, nFilters);
			break;
			
		case 4096:
			k_GPU_conv_OLS_R2R_via_customFFT<FFT_2048><<<gridSize, blockSize>>>(d_input_signal, d_output_plane, d_filters, signal_length, useful_part_size, h, offset, nConvolutions, nFilters);
			break;
			
		case 8192:
			k_GPU_conv_OLS_R2R_via_customFFT<FFT_4096><<<gridSize, blockSize>>>(d_input_signal, d_output_plane, d_filters, signal_length, useful_part_size, h, offset, nConvolutions, nFilters);
			break;
...
void convolution_via_customFFT_benchmark(float *d_input_signal, float *d_output_plane, float *d_filters_timedom, int signal_length, int convolution_length, int useful_part_size, float h, int offset, int nConvolutions, int nFilters, double *CONV_time){
	GpuTimer timer;
...
int GPU_convolution_OLS_customFFT(float *h_input_signal, float *h_output_plane, float *h_filters_timedom, int signal_length, int convolution_length, int filter_length, int past_filter_samples, int nFilters, int nRuns, float h, int offset_modifier, int device, double *execution_time){
	...
	//---------> CONV calculation
		...
		for(int f=0; f<nRuns; f++){
			checkCudaErrors(cudaMemcpy(d_input_signal, h_input_signal, input_size*sizeof(float), cudaMemcpyHostToDevice));
			checkCudaErrors(cudaMemcpy(d_filters_timedom, h_filters_timedom, filters_size*sizeof(float), cudaMemcpyHostToDevice));
			convolution_via_customFFT_benchmark(d_input_signal, d_output_plane, d_filters_timedom, signal_length, convolution_length, useful_part_size, h, offset, nConvolutions, nFilters, &total_CONV_kFFT_time);
			checkCudaErrors(cudaGetLastError());
		}
