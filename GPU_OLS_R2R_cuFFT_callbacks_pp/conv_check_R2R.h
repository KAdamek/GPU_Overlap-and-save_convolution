...
void CPU_postprocess(float *h_CPU_postprocessed, float *h_CPU_output_reduced, int nTimesamples, int nFilters, float h){
	...
	for(int f=0; f<nFilters; f++){
		for(int s=0; s<nTimesamples; s++){
			...
			if( s==(nTimesamples-1) ) {
				right = h_CPU_output_reduced[pos];
			}
			else {
				right = h_CPU_output_reduced[pos+1];
			}
			
			result = (left - right)/(2.0*h);
...
void Full_CONV_check(float *GPU_result, float *h_input_real, float *h_filters, int signal_length, int filter_length, int useful_part_size, int offset, int conv_length, int nConvolutions, int nFilters, float h, double *cumulative_error, double *mean_error){
	...
	float *h_CPU_postprocessed;
	h_CPU_output_timedomain = (float *)malloc(output_size_timedomain*sizeof(float));
	h_CPU_postprocessed = (float *)malloc(output_size_timedomain*sizeof(float));
