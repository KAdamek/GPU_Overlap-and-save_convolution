...
int GPU_CONV(float *h_input, float *h_output, float *h_filters_timedom, int signal_length, int filter_length, int nFilters, int nRuns, double *execution_time);
...
int main(int argc, char* argv[]) {
	...
	//----------------> GPU kernel
	int GPU_error = GPU_CONV(h_input, h_output, h_filters_padded, nTimesamples, filter_length, nFilters, nRuns, &execution_time);
	...
	if(CHECK){
		...
		Full_CONV_check(h_output, h_input, h_filters, nTimesamples, filter_length, useful_part_size, (filter_length>>1), CONV_SIZE, nConvolutions, nFilters, &total_error, &mean_error);
