...
int CONV_cuFFT_benchmark(float2 *d_input, float2 *d_extended_input, float2 *d_output, float2 *d_template, float2 *d_reduced_plane, int signal_length, int filter_length, int nFilters, double *CONV_time){
...
int GPU_CONV(float2 *h_input, float2 *h_output, float2 *h_filters, int signal_length, int filter_length, int nFilters, int nRuns, double *execution_time){
	...
	for(int r=0; r<nRuns; r++){
		...
		checkCudaErrors(cudaMemcpy(d_filters, h_filters, template_size*sizeof(float2), cudaMemcpyHostToDevice));
		Conv_error = CONV_cuFFT_benchmark(d_input_signal, d_input_signal_extended, d_output_plane, d_filters, d_output_plane_reduced, signal_length, filter_length, nFilters, &total_CONV_cuFFT_time);	
		if(Conv_error!=0) {
